const translations = {
  'pt-BR': {
    ...
    features: [
      ...
      ['Atualizações Frequentes', 'Novos fixes adicionados constantemente conforme os lançamentos.']
    ],
    fixesTitle: 'Fixes Populares',
    ...
    stable: 'Stable',
    testing: 'Testing',
    downloadFix: 'Baixar Fix',
    viewProject: 'Ver Projeto',
    gamesTitle: 'Jogos Compatíveis',
    ...
    faqTitle: 'FAQ',
    faq: [
      ['Os fixes são seguros?', 'Todos os projetos são testados pela comunidade antes de serem marcados como Stable.'],
      ['Quais GPUs possuem suporte?', 'RX 400/500 Series, GTX Pascal/Maxwell, Vega e algumas iGPUs.'],
      ['Posso sugerir um jogo?', 'Sim. A comunidade ajuda a escolher os próximos projetos.']
    ],
  ...
  en: {
    ...
    features: [
      ...
      ['Frequent Updates', 'New fixes are added constantly as new games are released.']
    ],
    fixesTitle: 'Popular Fixes',
    ...
    faqTitle: 'FAQ',
    faq: [
      ['Are the fixes safe?', 'All projects are tested by the community before being marked as Stable.'],
      ['Which GPUs are supported?', 'RX 400/500 Series, GTX Pascal/Maxwell, Vega and some iGPUs.'],
      ['Can I suggest a game?', 'Yes. The community helps choose the next projects.']
    ],
  ...
  ru: {
    ...
    features: [
      ...
      ['Частые обновления', 'Новые фиксы добавляются регулярно вместе с релизами игр.']
    ],
    fixesTitle: 'Популярные фиксы',
    ...
    stable: 'Стабильно',
    testing: 'Тестируется',
    downloadFix: 'Скачать фикс',
    viewProject: 'Открыть проект',
    gamesTitle: 'Совместимые игры',
    ...
    faqTitle: 'FAQ',
    faq: [
      ['Фиксы безопасны?', 'Все проекты тестируются сообществом перед тем, как получить статус Stable.'],
      ['Какие GPU поддерживаются?', 'RX 400/500 Series, GTX Pascal/Maxwell, Vega и некоторые iGPU.'],
      ['Можно предложить игру?', 'Да. Сообщество помогает выбирать следующие проекты.']
    ],
  ...
  es: {
    ...
    features: [
      ...
      ['Actualizaciones frecuentes', 'Nuevos fixes añadidos constantemente según los lanzamientos.']
    ],
    fixesTitle: 'Fixes populares',
    ...
    faqTitle: 'FAQ',
    faq: [
      ['¿Los fixes son seguros?', 'Todos los proyectos son probados por la comunidad antes de marcarse como Stable.'],
      ['¿Qué GPUs tienen soporte?', 'RX 400/500 Series, GTX Pascal/Maxwell, Vega y algunas iGPUs.'],
      ['¿Puedo sugerir un juego?', 'Sí. La comunidad ayuda a elegir los próximos proyectos.']
    ],
...
const setLanguage = (lang) => {
  ...
  setText('.badge', t.heroBadge);
  setText('.hero h2', t.heroTitle);
  setText('.hero p', t.heroText);
  setText('.hero-buttons .primary', t.explore);
  setText('.discord-hero', t.joinDiscord, 0);
  ...
  setText('.terminal pre', t.terminal);

  document.querySelectorAll('.card').forEach((card, index) => {
    if (!t.features[index]) return;
    const [title, text] = t.features[index];
  ...
  setText('#fixes .section-title h2', t.fixesTitle);
  setText('#fixes .section-title p', t.fixesSubtitle);
  document.querySelectorAll('.repo-card').forEach((card, index) => {
    const status = card.querySelector('.repo-top span');
    const desc = card.querySelector('p');
    if (status) status.textContent = index === 0 ? t.stable : t.testing;
    if (desc && t.repoDescriptions[index]) desc.textContent = t.repoDescriptions[index];
  });

  document.querySelectorAll('.download-link').forEach((link) => {
    link.innerHTML = `<span class="download-icon">⬇</span><span>${t.downloadFix}</span>`;
  });

  document.querySelectorAll('.repo-link').forEach((link) => {
    link.textContent = t.viewProject;
  });

  setText('#games .section-title h2', t.gamesTitle);
  setText('#games .section-title p', t.gamesSubtitle);
  document.querySelectorAll('.game-card').forEach((card, index) => {
    const status = card.querySelector('.game-status');
    const desc = card.querySelector('.game-info p');
    if (status) status.textContent = index === 0 ? t.stable : t.testing;
    if (desc && t.gameDescriptions[index]) desc.textContent = t.gameDescriptions[index];
  });

  setText('.community-box h2', t.communityTitle);
  setText('.community-box p', t.communityText);
  setText('.community-actions .primary', t.githubView);
  setText('.community-actions .discord-hero', t.joinDiscord);

  setText('#faq .section-title h2', t.faqTitle);
  document.querySelectorAll('.faq-item').forEach((item, index) => {
    if (!t.faq[index]) return;
    const [question, answer] = t.faq[index];
    const h4 = item.querySelector('h4');
    const p = item.querySelector('p');
    if (h4) h4.textContent = question;
    if (p) p.textContent = answer;
  });

  setText('footer .footer > div:first-child p', t.footer);
...
const revealOnScroll = () => {
  cards.forEach((card) => {
    ...
    const visiblePoint = window.innerHeight - 80;
...
const savedLanguage = localStorage.getItem('reviva-lang') || 'pt-BR';
